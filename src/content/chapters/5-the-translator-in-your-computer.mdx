---
chapter: 5
title: O Tradutor no seu Computador
shortname: Paginação
...
Até agora, sempre que eu falei sobre leitura e escrita de memória foi um pouco obscuro. Por exemplo, arquivos ELF espicificam endereços de memórias específicos para onde carregar dados, então por que não tem problemas com diferentes processos tendo conflitos no uso da memória Por que cada processo parece ter um ambiente de memória diferente?
...
Além disso, como exatamente a gente chegou aqui? Nós entendemos que o `execve` é uma chamada de sistema que *substitui* o processo atual por um novo programa, mas isso não explica muito como múltiplos processos podem ser iniciados. Isso definitivamente não explica como o primeiro programa executa - qual galinha (processo) bota (chama) os outros ovos (outros processos)?
...
Nós estamos chegando perto do fim da nossa jornada. Após ter as respostas para essas duas perguntas, nós teremos um quase total entendimento de como o computador chegou da inicialização até executar o programa que você está usando neste exato momento.
...
Então... sobre a memória. Acontece que quando a CPU lê ou escreve em um endereço de memória, ele não está de fato se referindo a essa localização na memória física (RAM). Em vez disso, está apontando para um espaço na memória *virtual*.
...
A CPU conversa com um chip chamado [unidade de gerenciamento de memória](https://en.wikipedia.org/wiki/Memory_management_unit) (*memory management unit* ou MMU). A MMU funciona como um tradutor com dicionário que traduz locais na memória virtual para locais na RAM. Quando a CPU recebe uma instrução para ler do endereço de memória `0xfffaf54834067fe2`, ele pede para a MMU traduzir esse endereço. A MMU olha no dicionário, descobre que o endereço físico correspondente é `0x53a4b64a90179fe2` e envia o número de volta para a CPU. A CPU então pode ler desse endereço na RAM.
...
<img src='/images/virtual-memory-mmu-example.png' loading='eager' style='max-width: 640px; margin: 0 auto;' alt='Um endereço mostrando uma CPU sorridente e uma MMU conversando. A MMU é um chip alto, usando óculos de leitura e está segurando um grande livro chamado "Dicionário: ponteiros 0x0000 até 0xffff". A CPU pede para a MMU traduzir um grande endereço de memória. A MMU pensa por um instante, e responde com um ponteiro diferente' width='1207' height='454' />
...
Quando o computador liga pela primeira vez, os acessos à memória vão diretamente para a RAM física. Imediatamente depois da inicialização, o sistema operacional cria um dicionário de tradução e diz para a CPU começar a usar a MMU.
...
Esse dicionário é na verdade chamado de *tabela de página*, e esse sistema de traduzir cada endereço de memória é chamado *paginação*. Entradas na tabela de página são chamadas *páginas* e cada página representa como um certo pedaço de memória virtual é mapeado para a RAM. Esses pedaços são sempre de um tamanho físico, e cada arquitetura de processador tem um tamanho de página diferente. O x86-64 tem por padrão 4KiB de tamanho de página, significando que cada página especifica o mapeamento para um bloco de memória de 4096 bytes.
...
Em outras palavras, com 4KiB de páginas os últimos 12 bits de um endereço vão ser sempre os mesmos tanto antes quanto depois da tradução da MMU - 12, porque esse é o número de bits necessários para indexar as 4096 páginas que você recebe depois da tradução.
...
A arquitetura x86-64 também permite que os sistemas operacionais usem páginas maiores de 2 MiB ou de 4 GiB, que podem melhorar a tradução de endereços mas aumentam a fragemntação de memória e seu desperdício. Quanto maior o tamanho da página, menor a porção de endereço que é traduzida pela MMU.
...
<img src='/images/4kib-paging-address-breakdown.png' loading='lazy' style='max-width: 400px; margin: 0 auto;' alt='Um fatiamento de um endereço de memória com paginação de 4 KiB. Os menores 12 bits indexam a página, enquanto o resto dos bits são traduzidos pela MMU e se transformam no endereço do começo da página.' width='760' height='310' />
...
A tabela de páginas por si só reside na RAM. Ainda que ela contenha milhões de entradas, o tamanho de cada entrada é na ordem de alguns bytes, então a tabela por si só não consome muito espaço.
...
Para habilitar a paginação no boot, o kernel primeiro constrói a tabela de páginas na RAM. Então, ele guarda os endereços físicos do começo da tabela de página em um registrador chamado registrador da base da tabela de página (*page table base register* ou PTBR). Por fim, o kernel habilita que a paginação traduza todos os acessos de memórias com a MMU. No x86-64, os primeiros 20 bits do registrador de controle 3 (CR3) funcionam como o PTBR. O bit 31 do CR0, designado PG (de paginação), recebe o valor 1 para habilitar a paginação.
...
A mágica da paginação é que cada tabela de página pode ser editada enquanto o computador está executando. É assim que cada processo pode ter seu próprio espaço de memória isolado - quando o OS muda de contexto de um processo para outro, uma importante tarefa é remapear o espaço de memória virtual para uma área diferente na memória física. Vamos dizer que você tem dois processos: o processo A pode ter seu código e seus dados (provavelmente carregados de um arquivo ELF!) no endereço `0x0000000000400000`, e o processo B pode acessar esse código e dados do mesmo endereço. Esses dois processos podem ser instâncias de um mesmo programa, porque eles não estão necessariamente brigando por essa faixa de endereço! Os dados para o processo A estão em algum lugar longe do processo B na memória física, e são mapeados para `0x0000000000400000`  na troca de processos.

<!-- NOTA: TA ESTRANHO ESSE ULTIMO PARAGRAFO -->
